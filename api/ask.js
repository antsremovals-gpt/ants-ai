export default async function handler(req, res) {
  ...
  try {
    ...
    // Detect user requests - includes both English and Romanian variations
    const askedForPhone = [
      "phone number", "contact number", "can i call", "what is your phone",
      "numar de telefon", "numarul de telefon", "număr de telefon", "numărul de telefon",
      "care este numarul vostru de telefon", "care este numărul vostru de telefon",
      "telefonul", "telefon"
    ].some(t => lastUserMessage.includes(t));

    const askedForEmail = [
      "email", "email address", "do you have an email", "what is your email", 
      "mail", "e-mail", "adresa de email", "care este emailul"
    ].some(t => lastUserMessage.includes(t));

    const askedForQuoteForm = [
      "quote", "get a quote", "quote form", "contact form", "request form",
      "formular", "cerere de oferta", "cerere de ofertă",
    ...
    // Check if user asked about price/cost - includes both English and Romanian
    ...
    // Responses for contact and links - English only
    if (providedPhone || providedEmail) {
      const x = providedEmail || providedPhone;
      return res.status(200).json({
        reply: `Thanks — we'll get back to ${x}. If you prefer another channel or a specific time, just say.`
      });
    }
    ...
    if (askedForQuoteForm) {
      const invite = "If you'd like an exact price, leave a phone number or email and we'll get back to you quickly.";
    ...
    // System message for OpenAI - English only
    const systemMessage = {
      role: "system",
      content: `
...
- Respond ONLY in English, regardless of the user's language.
...
- Containers are stacked 3 high, placed back-to-back with space for turning.
    ...
    // Contact invitation — only when price is requested
